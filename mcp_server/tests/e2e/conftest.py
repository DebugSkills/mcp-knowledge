...
async def e2e_http_app(real_qdrant, real_embedder, e2e_store, e2e_pipeline,
    ...
    # Раньше конструировался напрямую с дефолтным ttl=600.
    ...
    # code-2026-09-25-020 (P1): ДОЛЕГИРУЕМ реальному хендлеру main.py.
    # Прежний mock-роут всегда отвечал 200 {"status":"not_found"} и не
    # воспроизводил ни 404, ни self-heal scan_id → кейсы висячего указателя
    # давали ложную зелёнку. Реальный хендлер проверяет auth (запросы без
    # X-API-Key получают 401 — как на проде).
    from mcp_server.main import scan_progress as _real_scan_progress

    @app.get("/quality/scan/progress")
    async def scan_progress_route(request: Request):
        """13.15: прогресс фонового quality scan — реальный роут main.py."""
        return await _real_scan_progress(request)
